  ...
  <div class="flex bg-gray-100 h-screen">
    <aside v-show="isSidebarVisible"
      ...
      <ul class="p-4 space-y-2 ">
        <RouterLink to="/dashboard" class="flex p-1 ml-0 mb-3 rounded items-center gap-1 bg-teal-700">
          ...
          <div class="text-left p-3 text-lg font-bold rounded-md underline">Dashboard</div>
        </RouterLink>
    ...
    <div class="flex-1 p-4 overflow-hidden">
      ...
      <div class="flex-1 p-4 overflow-hidden">
        <!-- Header -->
        <h1 class="text-3xl font-semibold mb-8">Dashboard Perpustakaan</h1>
      </div>

      <!-- Grid Layout -->
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        <!-- Anggota -->
        <div class="bg-blue-500 text-white p-6 rounded-md shadow-md flex items-center">
          <font-awesome-icon :icon="['fas', 'users']" class="text-4xl mr-4" />
          <div class="justify-end">
            <h2 class="text-xl font-semibold mb-2">Anggota</h2>
            <div class="text-4xl font-bold">{{ memberList.length }}</div>
          </div>
        </div>

        <!-- Jenis Buku -->
        <div class="bg-green-500 text-white p-6 rounded-md shadow-md flex items-center">
          <font-awesome-icon :icon="['fas', 'book']" class="text-4xl mr-4" />
          <div>
            <h2 class="text-xl font-semibold mb-2">Total Buku</h2>
            <div class="text-4xl font-bold">{{ books.length }}</div>
          </div>
        </div>

        <!-- Peminjaman -->
        <div class="bg-yellow-500 text-white p-6 rounded-md shadow-md flex items-center">
          <font-awesome-icon :icon="['fas', 'clipboard-check']" class="text-4xl mr-4" />
          <div>
            <h2 class="text-xl font-semibold mb-2">Peminjaman</h2>
            <div class="text-4xl font-bold">{{ peminjamanList.length }}</div>
          </div>
        </div>

        <!-- Pengembalian -->
        <div class="bg-red-500 text-white p-6 rounded-md shadow-md flex items-center">
          <font-awesome-icon :icon="['fas', 'exchange-alt']" class="text-4xl mr-4" />
          <div>
            <h2 class="text-xl font-semibold mb-2">Pengembalian</h2>
            <div class="text-4xl font-bold">{{ pengembalianList.length }}</div>
          </div>
        </div>
      </div>
      ...
      <div class="mt-8 bg-white p-8 rounded-md shadow-md">
        ...
        <div class="overflow-x-auto">
          <table class="w-full table-auto">
            ...
            <tbody>
              <tr v-for="(book, index) in books" :key="index" class="bg-white">
                <td class="py-2 px-4">{{ book.Judul_Buku }}</td>
...
export default {
  ...
  setup() {
    ...
    return { memberList, bookList, peminjamanList, pengembalianList }
  },
  data() {
    return {
  ...
  mounted() {
    // Call the method to fetch books when the component is mounted
    this.fetchBooks();
  },
  methods: {
    toggleSidebar() {
    ...
    fetchBooks() {
      // Replace the URL with your API endpoint for fetching books
    ...
    confirmLogout() {
      // Your existing logout confirmation logic
